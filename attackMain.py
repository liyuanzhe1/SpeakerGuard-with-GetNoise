...
def parse_args():
    ...
    for system_type_parser in [iv_parser, xv_parser, audionet_c_parser]:
        ...
        fgsm_parser.add_argument('-max_iter', type=int, default=10) # PGD-10 default
        ...
        cwinf_parser.add_argument('-loss', type=str, choices=['Entropy', 'Margin'], default='Entropy')


        cw2_parser = subparser.add_parser("CW2")
        cw2_parser.add_argument('-initial_const', type=float, default=0.1)
        ...
        cw2_parser.add_argument('-lr', type=float, default=5e-3)
        cw2_parser.add_argument('-confidence', type=float, default=1.0)
        cw2_parser.add_argument('-save_noise', action='store_true', help='Save the adversarial noise')
        # cw2_parser.add_argument('-dist_loss', default='L2', choices=['L2', 'PMSQE', 'PESQ'])

        fakebob_parser = subparser.add_parser("FAKEBOB") # black box attack
        ...
        fakebob_parser.add_argument('-save_noise', action='store_true', help='Save the adversarial noise')

        siren_parser = subparser.add_parser("SirenAttack") # black box attack 
...
def save_audio(advers, names, root, fs=16000):
        for adver, name in zip(advers[:, 0, :], names):
            ...
            if isinstance(name, tuple):
                name = name[0]  # Assuming name is a tuple, take the first element
            spk_id = name.split("-")[0]
            spk_dir = os.path.join(root, spk_id)
            if not os.path.exists(spk_dir):
                os.makedirs(spk_dir)
            # Construct filename with global_index
            adver_path = os.path.join(spk_dir, f"{name}_{FAKEBOB.global_index}.wav")
            write(adver_path, fs, adver)
            FAKEBOB.global_index += 1  # Increment global_index for the next call
...
def main(args):
    ...
    model = base_model # use the model directly without defence
    spk_ids = base_model.spk_ids # list for speaker ids, not index 
